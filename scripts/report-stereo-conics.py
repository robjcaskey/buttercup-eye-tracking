...
def main():
    ...
    comparisons = []
    for eye, pairs in enumerate(heldout):
        comparisons.append({"eye": eye, "matched_reads": len(pairs),
            ...
            "regressed_over_1px": sum(a > b+1 for a, b in pairs)})
    expected = None
    ...
    report = {"schema": "buttercup-joint-conic-comparison-v1", "evaluation": str(args.evaluation),
        "scope": {"reads": rows, "both_raw_reads": both_raw, "both_evidence_reads": both_evidence,
                  ...
                  "complete_corpus": expected is not None and len(ids) == expected and not duplicates,
